...
// todo test
namespace conet {
template <typename T> class MpscQueue : boost::noncopyable {
public:
  MpscQueue();
  ~MpscQueue();
...
template <typename T> void MpscQueue<T>::enqueue(T &&input) {
...
template <typename T> void MpscQueue<T>::enqueue(const T &input) {
  BufferNode *node{new BufferNode(input)};
  BufferNode *prevhead{head_.exchange(node, std::memory_order_acq_rel)};
  prevhead->next_.store(node, std::memory_order_release);
}

template <typename T> bool MpscQueue<T>::dequeue(T &output) {
...
template <typename T> bool MpscQueue<T>::empty() {
  BufferNode *tail = tail_.load(std::memory_order_relaxed);
  BufferNode *next = tail->next_.load(std::memory_order_acquire);
  return next == nullptr;
}
