...
static size_t g_coStackSize =
    Config::Instance().getSize("coroutine", "stackSize", 128 * 1024);
...
Coroutine::Coroutine(const Callback &cb, std::string name, size_t stackSize)
    : state_(State::TERM), name_(name + "-" + std::to_string(coId_)), cb_(cb) {
  if (!t_main_co)
    ProduceMainCo();
  coId_ = ++s_coId;
  ++s_coNum;
  ...
  ctx_ = make_fcontext(static_cast<char *>(stack_) + stackSize_, stackSize_,
                       &Coroutine::RunInCo);
  DEBUG("Co <{}> created, coId = {}", name, s_coId);
}

Coroutine::Coroutine()
    : coId_(++s_coId), state_(State::EXEC),
...
void Coroutine::resume() {
  SetThisCo(this);
  if (state_ == State::TERM)
    return;
  // 保存主协程的上下文，恢复当前协程的上下文
...
void Coroutine::SetThisCo(Coroutine *co) { t_co = co; }
