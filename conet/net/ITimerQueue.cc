...
void resetTimerfd(int timerfd, Timestamp expiration) {
  ...
  newValue.it_value = detail::howMuchTimeFromNow(
      expiration); // 获取与现在的时间差值，然后设置关注事件
  int ret = ::timerfd_settime(timerfd, 0, &newValue, &oldValue);
  // timerfd_settime() return 0 on success; on error they return -1, and set
...
ITimerQueue::ITimerQueue() : timerfd_(detail::createTimerfd()) {}

ITimerQueue::~ITimerQueue() { ::close(timerfd_); }
