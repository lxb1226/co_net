...
#include "conet/net/ITimerQueue.h"
...
public:
  ...
  TimerId addTimer(TimerCallback cb, const std::string &name, Timestamp when,
                   double interval = 0);
...
private:
  ...
  // 保存的是取消的定时器 --- todo 此处数据结构可否优化 bool
  // callingExpiredTimers_;     // atomic, 是否正在处理超时事件 std::mutex
  // mutex_;
};
