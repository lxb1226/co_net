...
CoTimerQueue::CoTimerQueue(Processor *proc) : proc_(proc) {}

TimerId CoTimerQueue::addTimer(Coroutine::ptr co, Timestamp when,
                               double interval) {
  Timer::ptr timer = std::make_shared<Timer>(co, proc_, when, interval);
  bool earliestChanged = false;
  earliestChanged =
      insert(timer); // 是否将timer插入set首部，比现有队列里的所有的都更早
...
TimerId CoTimerQueue::addTimer(TimerCallback cb, const std::string &name,
...
void CoTimerQueue::dealWithExpiredTimer() {
  ...
  for (const auto &pair : expiredList) {
    ...
    if (oldTimer->getCo() != nullptr) {
      ...
      oldTimer->getProcessor()->addPendingTask(oldTimer->getCb(),
                                               oldTimer->getName());
    }
    if (oldTimer->getInterval() > 0) {
      ...
      if (oldTimer->getCo() != nullptr) {
        oldTimer->setCo(oldTimer->getProcessor()->resetAndGetCo(
            oldTimer->getCo()->getCallback(), "repeat"));
      } else {
