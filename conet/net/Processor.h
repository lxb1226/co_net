...
#include "conet/base/MpscQueue.h"
...
public:
  ...
  Coroutine::ptr resetAndGetCo(const Coroutine::Callback &cb,
                               const std::string &name);

  void addTask(Coroutine::ptr co);
  void addTask(const Coroutine::Callback &cb, const std::string &name = "");
  // 跨线程调用addTask
  void addPendingTask(const Coroutine::Callback &cb,
                      const std::string &name = "");
